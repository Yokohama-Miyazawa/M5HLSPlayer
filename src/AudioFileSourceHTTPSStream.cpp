/*
  ...
  This program was made by modifying "AudioFileSourceHTTPStream.cpp" of ESP8266Audio.
...
AudioFileSourceHTTPSStream::AudioFileSourceHTTPSStream(const char *url, const char *rootca)
{
  saveURL[0] = 0;
  reconnectTries = 0;
  open(url, rootca);
...
bool AudioFileSourceHTTPSStream::open(const char *url, const char *rootca)
{
  pos = 0;
  //http.begin(client, url);
  if (rootca == NULL || strstr(url, "https://") == NULL) {
    http.begin(url);  // HTTP
  } else {
    http.begin(url, rootca);  // HTTPS
  }
  http.setReuse(true);
#ifndef ESP32
