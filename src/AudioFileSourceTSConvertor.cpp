...
AudioFileSourceTSConvertor::AudioFileSourceTSConvertor(AudioFileSource *source, const bool isTSData)
{
  src = source;
  isInputTS = isTSData;
...
AudioFileSourceTSConvertor::~AudioFileSourceTSConvertor(){}

bool AudioFileSourceTSConvertor::seek(int32_t pos, int dir)
{
  return src->seek(pos, dir);
}

bool AudioFileSourceTSConvertor::close()
{
  return src->close();
}

bool AudioFileSourceTSConvertor::isOpen()
{
  return src->isOpen();
}

uint32_t AudioFileSourceTSConvertor::getSize()
{
  return src->getSize();
}

uint32_t AudioFileSourceTSConvertor::getPos()
{
  return src->getSize();
}
...
FileFormat AudioFileSourceTSConvertor::search()
{
  ...
  do {
    if(!isSyncByteFound) {
      uint8_t oneByte;
      do {
        if(!src->read(&oneByte, 1)) return FileFormat::UNKNOWN;
      } while (oneByte != 0x47);
      isSyncByteFound = true;
      read = src->read(&packetBuff[1], TS_PACKET_SIZE-1);
    } else {
...
void AudioFileSourceTSConvertor::parsePAT(uint8_t *pat)
{
...
void AudioFileSourceTSConvertor::parsePMT(uint8_t *pat)
{
...
int AudioFileSourceTSConvertor::parsePES(uint8_t *pat, const int posOfPacketStart, const bool isNewPayload, uint8_t *data)
{
  if (isNewPayload)
  {
...
int AudioFileSourceTSConvertor::parsePacket(uint8_t *packet, uint8_t *data, const bool isSearch)
{
  int read = 0;
...
uint32_t AudioFileSourceTSConvertor::read(void *data, uint32_t len)
{
  if(!isInputTS) return src->read(data, len);
  ...
  do {
    if(!isSyncByteFound) {
      uint8_t oneByte;
      do {
        if(!src->read(&oneByte, 1)) return 0;
      } while (oneByte != 0x47);
      isSyncByteFound = true;
      read = src->read(&packetBuff[1], TS_PACKET_SIZE-1);
    } else {
    ...
    if(read){ tsRead += parsePacket(packetBuff, &reinterpret_cast<uint8_t*>(data)[tsRead]); }
  } while ((len - tsRead) >= (TS_PACKET_SIZE - TS_HEADER_SIZE) && read);

  return tsRead;
}


bool AudioFileSourceTSConvertor::loop()
{
